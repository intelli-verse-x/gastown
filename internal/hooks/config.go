...
func SyncManagedClaudeSettings(target Target, dryRun bool) (SyncResult, error) {
	...
	expectedMCP, err := ComputeExpectedMCP(target.Key)
	if err != nil {
	...
	if fileExists && HooksEqual(expected, &current.Hooks) && HasClaudePromptDefaults(current) && HasExpectedMCP(current, expectedMCP) {
	...
	ApplyMCPToSettings(current, expectedMCP)
